	...
	*/
	...
	for (unsigned int i = 0; i<initialConditions.goals.size(); i++) {
			...
			initialConditions.goals[i].goalType == GOAL_TYPE_AXIS_ALIGNED_BOX_GOAL)
		{
			...
			controlPoints.push_back(Util::CurvePoint(_goalQueue.back().targetLocation, _goalQueue.back().targetTangent, _goalQueue.back().targetTime));
		}
		else {
...
void SocialForcesAgent::updateAI(float timeStamp, float dt, unsigned int frameNumber)
{
	...
	/*if (_position.vector()*(Util::Vector(1, 0, 0)) < 0)
	goalDirection.z += goalDirection.z - 0.1;
	else
	goalDirection.z += goalDirection.z + 0.025;*/
...
bool SocialForcesAgent::runLongTermPlanning()
{
	...
	Util::Point pos = position();
		...
		(astar.computePath(agentPath, pos, _goalQueue.front().targetLocation, gSpatialDatabase));
