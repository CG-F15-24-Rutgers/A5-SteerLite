	...
	*/
	...
	public:
		...
		*/
		...
		DO NOT CHANGE THE DEFINITION OF THIS FUNCTIONa
		...
		*/
		...
		double heuristic_cost_estimate(Util::Point start, Util::Point goal, int type);
		void reconstruct_path(AStarPlannerNode *current, std::vector<Util::Point>& agent_path);
		std::vector<AStarPlannerNode> neighbor_nodes(AStarPlannerNode* current, Util::Point goal, SteerLib::GridDatabase2D * _gSpatialDatabase, int type);
